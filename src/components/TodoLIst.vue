<template>
  <el-row class="content">
    <el-col :xs="{ span: 20, offset: 2 }" :sm="{ span: 8, offset: 8 }">
      ...
      ></el-input>
      <el-tabs v-model="activeName" @tab-click="getTodolist">
          ...
          <template>
            <template v-for="(item, index) in list">
              <div class="todo-list" v-if="item.status == true" :key="index">
                ...
                <span class="pull-right">
                  <el-button size="small" type="primary" @click="update(index)"
                    >还原</el-button
                  >
                </span>
              </div>
...
export default {
  ...
  methods: {
    ...
    getTodolist() {
      const status = this.activeName === "first" ? 0 : 1;
      this.$http
        .get("/api/todolist/" + this.id + '/' + status) // 向后端发送获取todolist的请求
        .then(
          res => {
            if (res.status == 200) {
